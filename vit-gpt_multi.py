...
class Flickr8kDataset(torch.utils.data.Dataset):
    ...
    # DistributedSampler 정의
    train_sampler = DistributedSampler(processed_train, shuffle=True)
    val_sampler = DistributedSampler(processed_val, shuffle=False)
    test_sampler = DistributedSampler(processed_test, shuffle=False)

    # Create data loaders with DistributedSampler
    train_loader = DataLoader(Flickr8kDataset(processed_train, feature_extractor, tokenizer),
                              batch_size=16, 
                              num_workers=0, 
                              pin_memory=True, 
                              sampler=train_sampler)
    
    val_loader = DataLoader(Flickr8kDataset(processed_val, feature_extractor, tokenizer),
                            batch_size=16, 
                            num_workers=0, 
                            pin_memory=True, 
                            sampler=val_sampler)
    
    test_loader = DataLoader(Flickr8kDataset(processed_test, feature_extractor, tokenizer),
                             batch_size=16, 
                             num_workers=0, 
                             pin_memory=True, 
    ...
    def train_model(model, train_loader, val_loader, optimizer, num_epochs=10, patience=3):
        ...
        if local_rank == 0:
            ...
            model.module.save_pretrained("/home/jupyter/vit-gpt-2")
