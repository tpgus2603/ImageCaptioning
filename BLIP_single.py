...
class Flickr8kDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Move tensors to GPU (if necessary) - this should be done in collate_fn or later
        ...
        return {
            ...
            "caption": caption  # Include the caption in the returned dictionary
        }
...
def custom_collate_fn(batch):
    ...
    captions = [item['caption'] for item in batch]  # Extract captions
...
# DataLoader instances with custom collate function
train_loader = DataLoader(Flickr8kDataset(processed_train, processor), batch_size=4, shuffle=True, num_workers=0, collate_fn=custom_collate_fn)
val_loader = DataLoader(Flickr8kDataset(processed_val, processor), batch_size=4, shuffle=False, num_workers=0, collate_fn=custom_collate_fn)
test_loader = DataLoader(Flickr8kDataset(processed_test, processor), batch_size=4, shuffle=False, num_workers=0, collate_fn=custom_collate_fn)
...
train_model(model, train_loader, val_loader, optimizer, lr_scheduler, num_epochs=10, patience=3)
